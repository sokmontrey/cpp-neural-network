 ...
 *
 * Layer input(2);
 * Layer hidden(2, 3, input);
 * Layer output(3, 2, hidden);
*/

class Layer{

using NodeType = std::variant<Node, Node*>;
protected:
    NodeType value;
...
public:
    ...
    Layer(size_t input_size){
        value = Node::Vector(input_size);

        delete weight;
        delete bias;
    }
    Layer(size_t input_size, size_t output_size, Layer& previous){
